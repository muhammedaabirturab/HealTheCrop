...
def _to_response(record: Prediction, result: dict, season: str) -> PredictionResponse:
    return PredictionResponse(
        ...
        crop_details=result["crop_details"] or {
            ...
            "image": "placeholder.jpg", "season": [season], "water_requirement": "Medium",
...
def predict_manual(
    ...
    season = resolve_season(payload.season)
    predictor = get_crop_predictor()
    result = predictor.predict(features, season, payload.location or user.location or "Karnataka")

    record = Prediction(
        user_id=user.id, input_features=features, recommended_crop=result["recommended_crop"],
        confidence=result["confidence"], alternatives=result["alternatives"], season=season, source="manual",
    )
    db.add(record)
    db.commit()
    db.refresh(record)
    return _to_response(record, result, season)
...
def predict_from_device(
    ...
    record = Prediction(
        user_id=user.id, input_features=features, recommended_crop=result["recommended_crop"],
        confidence=result["confidence"], alternatives=result["alternatives"], season=season, source="sensor",
    )
    db.add(record)
    db.commit()
    db.refresh(record)
...
def prediction_history(limit: int = 50, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    for record in records:
        ...
        responses.append(PredictionResponse(
            ...
            crop_details=details, season_used=record.season, created_at=record.created_at,
        ))
    return responses
